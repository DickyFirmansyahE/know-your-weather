...
const FORECAST_URL = "https://api.openweathermap.org/data/3.0";
...
const getForeCastData = (infoType, searchParams) => {
  const url = new URL(FORECAST_URL + "/" + infoType);
  url.search = new URLSearchParams({ ...searchParams, appid: API_KEY });
...
const formatCurrentWeather = (data) => {
  const {
    coord: { lat, lon },
    main: { temp, feels_like, temp_min, temp_max, humidity, min, max },
    name,
    dt,
